...
import {
  ...
  Zap, Gift, Heart, Trophy, Timer, Radio, Shuffle, Repeat,
  ChevronDown, Sparkles, ListMusic, Settings
} from "lucide-react";
                    ...
                    <div className="mb-3">
                      <div className="h-1 bg-muted/40 rounded-full overflow-hidden">
                        <motion.div className="h-full rounded-full" style={{ background: SPOTIFY_GREEN }}
                          animate={isPlaying ? { width: ["33%", "66%"] } : {}}
                          transition={isPlaying ? { duration: 30, ease: "linear" } : {}}
                          initial={{ width: "33%" }}
                        />
                      </div>
                      <div className="flex justify-between mt-1 text-[10px] text-muted-foreground">
                        <span>1:08</span><span>3:24</span>
                      </div>
                    </div>
                      ...
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                        ...
                        <div className="p-5 space-y-3">
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              ...
                              <select className="w-full appearance-none bg-muted/30 border border-border/30 rounded-lg px-3 py-2 text-sm text-foreground outline-none focus:ring-1 focus:ring-primary">
                                <option>🎁 Gift Rose</option>
                                <option>💎 Gift Diamond</option>
                                <option>🏀 Gift Basketball</option>
                                <option>❤️ 100 Hearts</option>
                            ...
                            <div>
                              ...
                              <select className="w-full appearance-none bg-muted/30 border border-border/30 rounded-lg px-3 py-2 text-sm text-foreground outline-none focus:ring-1 focus:ring-primary">
                                <option>▶ Play Track</option>
                                <option>📋 Play Playlist</option>
                                <option>⏭ Skip Track</option>
                                <option>🔊 Volume Boost</option>
                                <option>🎵 Fade In Music</option>
                                <option>🔇 Fade Out Music</option>
                              </select>
                            </div>
                          </div>
                          <div>
                            ...
                            <div className="flex gap-2">
                              ...
                              <button className="p-2 rounded-lg bg-muted/30 border border-border/30 text-muted-foreground hover:text-foreground transition-colors">
                                <Search size={14} />
                              </button>
                            </div>
                          </div>
                          ...
                          <div className="flex justify-end gap-2 pt-1">
                            <button onClick={() => setShowNewTrigger(false)} className="px-4 py-2 rounded-lg text-xs text-muted-foreground hover:text-foreground transition-colors">
                              Cancel
                            </button>
                            <button
                              onClick={() => { setShowNewTrigger(false); toast.success("Music trigger saved!"); }}
                    ...
                    return (
                      <div key={item.path} className="flex items-center gap-4 px-5 py-3.5 border-b border-white/[0.03] last:border-0">
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                          style={{ background: "hsl(141 73% 42% / 0.1)" }}>
