...
interface AlertEvent {
  ...
  avatar?: string;
  giftImageUrl?: string;
  // Per-gift overrides from user_gift_triggers
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("broadcast", { event: "gift_alert" }, (msg) => {
        ...
        // Check if this gift has a per-gift trigger with overrides
        const trigger = giftTriggers.find(t => t.gift_id === giftId);
        ...
        const event: AlertEvent = {
          ...
          animationOverride: trigger?.animation_effect && trigger.animation_effect !== "bounce" ? trigger.animation_effect : undefined,
          soundOverride: trigger?.alert_sound_url || undefined,
        };
      ...
      <AnimatePresence>
        {alerts.slice(0, s.max_on_screen).map(alert => {
          ...
          return (
            ...
            >
              {/* Expanding ring - hidden when no border */}
              {!noBorder && (
                <motion.div
              ...
              {/* Glow - hidden when no background */}
              {!noBackground && (
                <motion.div
